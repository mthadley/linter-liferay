...
function getConfigPath(filePath) {
  const configFile = (0, findCached)(filePath, ['.csfrc.js', '.csfrc.yaml', '.csfrc.yml', '.csfrc.json', '.csfrc', 'csf.config.js']);
  if (configFile) {
    return true;
  }

  const packagePath = (0, findCached)(filePath, 'package.json');
  if (packagePath && Boolean(require(packagePath).csfConfig)) {
    return true;
  }
  return null;
}
  ...
  constructor() {
    ...
    this._subscriptions.add(
      atom.workspace.onDidOpen(event => {
        ...
        if (!(uri in this._pathMap)) {
          if (getConfigPath(uri)) {
            this._pathMap[uri] = true
          }
          else {
            this._pathMap[uri] = false
          }
        }
      })
    )
